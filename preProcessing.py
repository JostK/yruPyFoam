...
def findCellNumberLast(distG, lastCell, growRate):
    """
    function to get Number of cells based on grow rate, maximum size of wall cell and general distance
    input:
        distG:        general distance
    ...
    #if growRate<1.0: raise ValueError("please provide grow rate >1")
    #guess=(distG/firstCell)/growRate**(distG/firstCell)
    # firstCell = lastCell/growRate**(N-1)
    func = lambda N : (1 -growRate**N) / (1 - growRate) - growRate**(N-1) * distG / lastCell    
    [N] = fsolve(func, 100)
    #as a full number of cells is needed and firstCell represents a maximum distance, the number is rounded up
    N = np.ceil(N)
    # resulting size of first cell
...
def gradingCalculator(firstCell=None, lastCell=None, nCells=None, distance=None, growRate=None, grading=None):
    """
    ...
    distG = sum_{n=0}^{N-1} firstCell*growRate^n => distG/firstCell =  sum_{n=0}^{N-1} growRate^n = (1 - growRate^N) / (1-growRate)
    """
    # Pre-calculations if grading is given instead of growRate:
    if grading is not None:
        if (nCells is not None) and (growRate is not None):
            raise KeyError(f"Information provided through the 'grading' parameter is redundant")
        elif nCells is not None:
            growRate = grading**(1.0/(nCells-1.0))
        elif growRate is not None:
            nCells = np.log(grading) / np.log(growRate) + 1
            
        if (lastCell is not None) and (firstCell is not None):
            raise KeyError(f"Information provided through the 'grading' parameter is redundant")
        elif lastCell is not None:
                firstCell = lastCell/grading
        elif firstCell is not None:
                lastCell = firstCell * grading
    
    #print(f"Grading calculation with firstCell = {firstCell}, lastCell = {lastCell}, nCells = {nCells}, distance = {distance}, growRate = {growRate}")
    
    found = False
    
    if (firstCell is not None) and (lastCell is not None):
        if nCells is not None:
            ...
            found = True
    elif (distance is not None) and (growRate is not None):
        if firstCell is not None:
            nCells, firstCell, lastCell = findCellNumberFirst(distance, firstCell, growRate)
            found = True
        elif lastCell is not None:
            nCells, firstCell, lastCell = findCellNumberLast(distance, lastCell, growRate)
            found = True
        elif nCells is not None:
            firstCell =  distance / ((1-growRate**nCells)/(1-growRate))
            nCells, firstCell, lastCell = findCellNumberFirst(distance, firstCell, growRate)
            found = True
    elif (distance is not None) and (nCells is not None):
        if firstCell is not None:
